...
export class QueryBox extends React.Component {//定义一个LeftNav组件,类定义法替代React.createClass 方法来定义一个组件
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.setState({
            orignList: orignList,
            pageList: pageList,
            selectName: {name: pageList[0].defaultSelect}
        })
    }
    ...
    checkBox(obj, index) {
        return (
            <div key={index + 'div'} className="ipt-checkbox" onClick={(e) => this.checkClick(obj)}>
    ...
    radio(obj, index) {
        return (
            <div key={index + 'div'} className="ipt-radio" onClick={(e) => this.radioClick(index,obj)}>
    ...
    textList(obj, index) {
        ...
        return (
            <div key={index + 'div'} className="ipt-text">
                <label key={index + 'label'} className="ipt-name">{obj.name}</label>
                <input key={index + 'input'} type="text" className="ipt-text" placeholder={obj.text}
    ...
    queryStyle() {
        //查询按钮暂可写死
        let iptbutton = <input type="button" key='input' className="ipt-button" value="查询"
        ...
        pageList.map((obj, index) => {
            switch (obj.dataType) {
                ...
                case "text":
                    ...
                    break;
                case "select":
                    querybox.push(<DropdownMenu dropdownMsg={obj.dropdownList} dropdownReturn={(id, desc) => this.outputId(id, desc)} defaultSelect={this.state.selectName.name}/>);
                    break;
                case "date":
